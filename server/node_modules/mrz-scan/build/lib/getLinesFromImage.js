...
function getMask(image, maskOptions) {
  let mask = new ImageClass(image.width, image.height, {
    kind: 'BINARY'
  });
  const partsY = 1;
  const partsX = 30;
...
module.exports = function getLinesFromImage(image) {
  ...
  rois.forEach(function (roi) {
    ...
    mbr = mbr.map(point => [point[0] + mask.position[0], point[1] + mask.position[1]]);
    painted.paintPolyline(mbr, {
      color: [255, 0, 0]
    });
  });
  return {
